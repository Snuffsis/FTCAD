...
public class JSONValue
{
	...
	{
		return (paramObject == null) ? "null"
				: ((paramObject instanceof String) ? ("\"" + escape((String)paramObject) + "\"")
						: ((paramObject instanceof Double)
								? ((((Double)paramObject).isInfinite() || ((Double)paramObject).isNaN()) ? "null"
										: paramObject.toString())
								: ((paramObject instanceof Float)
										? ((((Float)paramObject).isInfinite() || ((Float)paramObject).isNaN()) ? "null"
												: paramObject.toString())
										: ((paramObject instanceof Number) ? paramObject.toString()
												: ((paramObject instanceof Boolean) ? paramObject.toString()
														: ((paramObject instanceof JSONAware)
																? ((JSONAware)paramObject).toJSONString()
																: ((paramObject instanceof Map)
																		? JSONObject.toJSONString((Map)paramObject)
																		: ((paramObject instanceof List)
																				? JSONArray
																						.toJSONString((List)paramObject)
																				: paramObject.toString()))))))));
	}
